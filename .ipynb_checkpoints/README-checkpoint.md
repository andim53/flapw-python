...
We developed **FLAPW-PYTHON Wrapper** to provide a Python interface for running FLAPW commands.
...
| **`ls`**    | **L**i**s**t directory contents                                                  | `ls -l` (lists details)                           |
| **`cd`**    | **C**hange **D**irectory                                                         | `cd Documents` or `cd ..` (go up one level)       |
| **`mkdir`** | **M**ake **D**irectory (Create a new folder)                                     | `mkdir MyFolder`                                  |
| **`touch`** | Create an empty file                                                             | `touch notes.txt`                                 |
| **`cat`**   | Display file contents                                                            | `cat notes.txt`                                   |
| **`cp`**    | **C**o**p**y files or directories                                                | `cp old.txt new.txt`                              |
| **`mv`**    | **M**o**v**e or **R**e**n**ame files/directories                                 | `mv old.txt archive/` or `mv old.txt renamed.txt` |
| **`rm`**    | **R**e**m**ove (delete) files                                                    | `rm badfile.txt`                                  |
| **`rm -r`** | **R**emove directories (requires recursive flag)                                 | `rm -r MyFolder`                                  |
| **`man`**   | Show the **man**ual page for any command                                         | `man ls`                                          |
| **`sudo`**  | **S**uper **U**ser **Do** (Execute a command with root/administrator privileges) | `sudo apt update`                                 |
| **`chmod`** | **Ch**ange **Mod**e (Change file permissions)                                    | `chmod +x script.sh` (makes a file executable)    |
| **`apt`**   | Package manager for Debian/Ubuntu (Install software)                             | `sudo apt install firefox`                        |
## FLAPW COMMAND

| **Command**   | **Function**             | **Description**                                                                            | **Example Usage** |
| ------------- | ------------------------ | ------------------------------------------------------------------------------------------ | ----------------- |
| **`FLchk`**   | **Check Results**        | Checks and displays the result of a DFT calculation.                                       | `FLchk`           |
| **`FLclean`** | **Clean/Remove Results** | Removes all files generated by the DFT calculation.                                        | `FLclean`         |
| **`FLcopy`**  | **Copy and Tag**         | Copies the most recent result files and adds a specified tag (e.g., `SCF`) to their names. | `FLcopy SCF`      |
| **`FLrst`**   | **Remove Tag**           | Removes a specified tag (e.g., `SCF`) from the names of the DFT result files.              | `FLrst SCF`       |

## ADDING FLAPW COMMAND INTO PATH
### MAC/LINUX
**Add the bin into path**
```
CURRENT_LOCATION=$(pwd) && echo "Current Location (PWD): $CURRENT_LOCATION" && export PATH="$CURRENT_LOCATION/bin:$PATH" && echo -e "\nNew PATH (showing the new entry at the start):" && echo $PATH
```

### WINDOWS (PowerShell)
**Add the bin into path**
```
$CURRENT_LOCATION = Get-Location; Write-Host "Current Location: $CURRENT_LOCATION"; $env:Path = "$CURRENT_LOCATION\bin;$env:Path"; Write-Host $env:Path
```

### WINDOWS (cmd)
**Add the bin into path**
```
echo Current Location: %CD%; set PATH=%CD%\bin;%PATH%; echo.; echo New PATH (showing the new entry at the start):; echo %PATH%
```
